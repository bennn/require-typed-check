...
(require
  ...
  (rename-in tagged/racket
    [require/typed require/tagged])
  (rename-in (only-in tagged/racket/no-check require/typed)
    [require/typed require/tagged/no-check]))
  ...
  (define-syntax-class (clause lib)
    #:attributes (ann req)
      (pattern oc:opaque-clause
        #:attr ann #'#f
        #:attr req #`(require/tagged #,lib (#:opaque oc.ty oc.pred . oc.opt)))
   ...
   [(_ lib clause* ...)
    (begin
      (if (not (typed-lib? #'lib))
        ...
        (syntax/loc stx (require/tagged lib clause* ...))
